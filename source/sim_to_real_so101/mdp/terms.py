...
def any_vial_grasped(
    ...
    for vial_idx, vial_name in enumerate(vials):
        ...
        vial_z = as_torch(vial.data.root_pos_w)[:, 2]  # Z position per environment
...
def vial_placed_on_rack(
    ...
    rack_pos_w = as_torch(rack_obj.data.root_pos_w)       # (num_envs, 3)
    rack_quat_w = as_torch(rack_obj.data.root_quat_w)     # (num_envs, 4)
    ...
    for vial_idx, vial_name in enumerate(vials):
        vial: RigidObject = env.scene[vial_name]
        vial_pos_w = as_torch(vial.data.root_pos_w)       # (num_envs, 3)
        vial_quat_w = as_torch(vial.data.root_quat_w)     # (num_envs, 4)
...
def vial_placed_on_rack_termination(
    ...
    if env._rack_confirm_active.any():
        ...
        for vial_idx, vial_name in enumerate(vials):
            ...
            vial_pos_w = as_torch(vial_obj.data.root_pos_w)
            vial_quat_w = as_torch(vial_obj.data.root_quat_w)
