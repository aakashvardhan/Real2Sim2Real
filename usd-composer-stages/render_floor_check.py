...
renderer) and captures a top-down-ish view of /World/Floor after restyling
it (Cube->Mesh + new diffuse/roughness/normal texture set), so the new look
...
be accumulating (this blew out an earlier attempt that DID call play()).
Render with physics stopped, matching render_wrist_camera_check.py's approach.

Run with Isaac Sim's own Python (NOT usdenv, which has no Hydra/RTX imaging):
    C:\\Isaac-Sim\\python.bat usd-composer-stages\\render_floor_check.py
"""
...
OUT_PATH = (
    ...
    r"\usd-composer-stages\floor_material_check.png"
)
OUT_DIR = tempfile.mkdtemp(prefix="floor_check_")
CAMERA_PATH = "/World/FloorCheckCamera"
...
eye = np.array([0.3, -1.6, 3.2])
target = np.array([0.1, 0.3, 0.0])
...
xformable.AddOrientOp(UsdGeom.XformOp.PrecisionDouble).Set(
    __import__("pxr").Gf.Quatd(float(quat[0]), float(quat[1]), float(quat[2]), float(quat[3]))
)
